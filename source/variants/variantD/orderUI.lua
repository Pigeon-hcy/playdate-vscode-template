...
import "variants/variantD/orderRuntime"
...
function OrderUI.draw()
    -- Variant D reserves the top strip for its crank-cooking progress bar.
    local yOffset = PlayerConfig.activeWorkstation == 2 and 34 or 0
    ...
    gfx.fillRect(holderX - 2, yOffset, holderWidth + 4, uiHeight + 1)
    gfx.setClipRect(holderX, yOffset, holderWidth, uiHeight)
    ...
    for _, order in ipairs(Orders.pending) do
        ...
        local y = OrderUI.getTicketY(order) + yOffset
        receipt:draw(x, y)
    ...
    -- The holder hangs from the screen top, or just below the crank-grill bar;
    -- tickets still retract behind its front lip as they age.
    holder:draw(holderX, holderY + yOffset)
    gfx.popContext()
end
