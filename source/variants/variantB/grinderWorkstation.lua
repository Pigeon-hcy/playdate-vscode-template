...
import "variants/variantB/burstDialog"
import "variants/variantB/minceCounter"
...
-- Simple version guidance. These stay on the grinder until the requested
-- action is taken, so the player never has to remember a timed tutorial.
local guideArrowWidth <const> = 24
local guideArrowHeight <const> = 15
local function makeGuideDialog(title, prompt, direction)
    local titleWidth = bigFont:getTextWidth(title)
    local promptWidth = smallFont:getTextWidth(prompt)
    return BurstDialog.new({
        width = math.max(titleWidth, promptWidth, guideArrowWidth),
        height = bigFont:getHeight() + 4 + smallFont:getHeight() + 5 + guideArrowHeight,
        draw = function(width)
            local y = 0
            bigFont:drawText(title, math.floor((width - titleWidth) / 2), y)
            y += bigFont:getHeight() + 4
            smallFont:drawText(prompt, math.floor((width - promptWidth) / 2), y)
            y += smallFont:getHeight() + 5
            gfx.setColor(gfx.kColorBlack)
            local centerX = width / 2
            if direction == "down" then
                gfx.fillTriangle(centerX - guideArrowWidth / 2, y,
                    centerX + guideArrowWidth / 2, y,
                    centerX, y + guideArrowHeight)
            else
                local middleY = y + guideArrowHeight / 2
                gfx.fillRect(centerX - guideArrowWidth / 2, middleY - 3,
                    guideArrowWidth - 8, 7)
                gfx.fillTriangle(centerX + guideArrowWidth / 2, middleY,
                    centerX + guideArrowWidth / 2 - 9, y,
                    centerX + guideArrowWidth / 2 - 9, y + guideArrowHeight)
            end
        end,
    })
end

local guideDialogs <const> = {
    load = makeGuideDialog("NO MEAT!", "PRESS DOWN TO LOAD", "down"),
    stove = makeGuideDialog("300 MINCE!", "PRESS RIGHT FOR STOVE", "right"),
}
local guideMode = nil
local guideDialog = nil
local stoveGuideTriggered = false
local stoveGuideCompleted = false

local function setGuide(mode, animateExit)
    if guideMode == mode then return end
    if guideDialog ~= nil then
        if mode == nil and animateExit then
            guideDialog:hide(juice.time)
        else
            guideDialog:cancel()
        end
    end
    guideMode = mode
    if mode ~= nil then
        guideDialog = guideDialogs[mode]
        guideDialog:show(juice.time)
    end
end

local function syncGuide(isActive)
    if PlayerConfig.mincedMeat >= grinder.stovePromptThreshold then
        stoveGuideTriggered = true
    end

    if not isActive then
        if guideMode == "stove" and PlayerConfig.activeWorkstation == 2 then
            stoveGuideCompleted = true
        end
        setGuide(nil, false)
        return
    end
    ...
    if jamActive then
        setGuide(nil, false)
    elseif stoveGuideTriggered and not stoveGuideCompleted then
        setGuide("stove", false)
    elseif not grinder.hasMeat then
        setGuide("load", false)
    else
        setGuide(nil, true)
    end
...
function GrinderWorkstation.loadNewMeat()
    ...
    syncGuide(PlayerConfig.activeWorkstation == 1)
...
function GrinderWorkstation.initialize()
    ...
    jamCount = 0
    guideMode = nil
    if guideDialog ~= nil then guideDialog:cancel() end
    guideDialog = nil
    stoveGuideTriggered = false
    stoveGuideCompleted = false
    GrinderWorkstation.resetJamSchedule()
    jamReadyAt = juice.time
    jamMeatCounted = false
    grinder.hasMeat = false
    grinder.totalYield = 0
    grinder.producedYield = 0
    grinder.processedDegrees = 0
    juice:remove(meatEffectId)
    syncGuide(PlayerConfig.activeWorkstation == 1)
...
function GrinderWorkstation.getGuideMode()
    return guideMode
end

function GrinderWorkstation.isGuideVisible()
    return guideDialog ~= nil and guideDialog:isVisible(juice.time)
end

function GrinderWorkstation.hasCompletedStoveGuide()
    return stoveGuideCompleted
end
...
function GrinderWorkstation.draw()
    ...
    elseif guideMode == "stove" then
        gfx.drawText("➡  GO TO STOVE", 12, 212)
    ...
    if not jamActive and guideDialog ~= nil then
        guideDialog:draw(245, 150, juice.time)
    end
end
