...
import "variants/variantC/supplyWarning"
import "variants/variantC/juicy"
import "variants/variantC/juiceRuntime"
import "variants/variantC/orderRuntime"
...
-- Shown on the controls card; keep in step with the QTE map below.
AssemblyWorkstation.help = {
    title = "ASSEMBLY QTE",
    summary = "FOLLOW EACH PROMPT TO BUILD THE RECIPE IN ORDER, THEN FINISH AND SERVE WITH THE SHOWN COMBOS.",
    controls = {
        { "🎣", "SHAKE WHEN THE PROMPT SHOWS IT" },
        { "Ⓐ Ⓑ ⬇", "PRESS OR COMBINE AS SHOWN" },
        { "⬆ + Ⓐ", "PUT THE TOP BUN ON" },
...
local qteInputs = {}
local qteInputStartedAt = nil
local crankMotion = 0
local crankStartedAt = nil
...
local function resetQteInput()
    qteInputs = {}
    qteInputStartedAt = nil
    crankMotion = 0
    crankStartedAt = nil
end

local function currentQteStep()
    if servePhase ~= nil or completionPending then return nil end

    local recipe = PlayerConfig.recipe[assembly.currentRecipeIndex]
    if #assembly.layers < #recipe - 1 then
        local ingredientCode = recipe[#assembly.layers + 2]
        local action = assembly.qte.ingredients[ingredientCode]
        assert(action, "missing assembly QTE for " .. ingredientCode)
        return {
            kind = "ingredient",
            code = ingredientCode,
            name = assembly.ingredientNames[ingredientCode],
            tokens = action.tokens,
            label = action.label,
        }
    end

    local action = assembly.hasTopBread and assembly.qte.serve
        or assembly.qte.topBun
    return {
        kind = assembly.hasTopBread and "serve" or "topBun",
        name = assembly.hasTopBread and "SERVE" or "TOP BUN",
        tokens = action.tokens,
        label = action.label,
    }
end

function AssemblyWorkstation.getQteStep()
    return currentQteStep()
end

function AssemblyWorkstation.getQteProgress()
    local count = 0
    for _ in pairs(qteInputs) do count += 1 end
    return count
end

local function qteExpects(step, token)
    for _, expected in ipairs(step.tokens) do
        if expected == token then return true end
    end
    return false
end

local function completeQteStep(step)
    resetQteInput()
    if step.kind == "ingredient" then
        if AssemblyWorkstation.addIngredient(step.code) then return "complete" end
        juice:errorShake("error")
        return "failed"
    end

    local result = AssemblyWorkstation.pressUp()
    if result == "closed" or result == "correct" or result == "wrong" then
        return "complete"
    end
    return result
end

function AssemblyWorkstation.acceptQteToken(token)
    local step = currentQteStep()
    if step == nil then return "unavailable" end

    if qteInputStartedAt ~= nil and
        juice.time - qteInputStartedAt > assembly.qte.comboWindow then
        resetQteInput()
    end

    if not qteExpects(step, token) then
        resetQteInput()
        juice:errorShake("error")
        return "wrong"
    end

    if qteInputStartedAt == nil then qteInputStartedAt = juice.time end
    qteInputs[token] = true
    for _, expected in ipairs(step.tokens) do
        if not qteInputs[expected] then return "partial" end
    end

    return completeQteStep(step)
end
...
function AssemblyWorkstation.handleInput()
    -- Always consume crank movement while this workstation is active, even
    -- during its completion and serving animations.
    local crankChange = math.abs(pd.getCrankChange())
    if currentQteStep() == nil then return end

    if crankChange > 0 then
        if crankStartedAt == nil or
            juice.time - crankStartedAt > assembly.qte.comboWindow then
            crankMotion = 0
            crankStartedAt = juice.time
        end
        crankMotion += crankChange
        if crankMotion >= assembly.qte.crankDegrees then
            local result = AssemblyWorkstation.acceptQteToken("CRANK")
            crankMotion = 0
            crankStartedAt = nil
            if result ~= "partial" then return end
        end
    end
    ...
    for _, buttonToken in ipairs(buttonTokens) do
        if pd.buttonJustPressed(buttonToken[1]) then
            local result = AssemblyWorkstation.acceptQteToken(buttonToken[2])
            if result ~= "partial" then return end
        end
    end
end
...
function AssemblyWorkstation.update(isActive)
    ...
    if not isActive then
        resetQteInput()
    elseif qteInputStartedAt ~= nil and
        juice.time - qteInputStartedAt > assembly.qte.comboWindow then
        resetQteInput()
    elseif crankStartedAt ~= nil and
        juice.time - crankStartedAt > assembly.qte.comboWindow then
        crankMotion = 0
        crankStartedAt = nil
    end
...
local function drawCentered(text, y, font)
    ...
    local panelWidth = PlayerConfig.screenWidth - ingredientPanelLeft
    gfx.drawText(text, ingredientPanelLeft + math.floor((panelWidth - width) / 2), y)
end

local function drawQtePanel()
    drawCentered("QTE", 7, systemFont)
    local step = currentQteStep()
    if step == nil then
        local status = servePhase ~= nil and
            (servePhase == "outgoing" and "SERVING..." or "NEXT...")
            or "FINISHING..."
        drawCentered(status, 88, smallFont)
        return
    end

    drawCentered(step.name, 40, smallFont)
    drawCentered("DO THIS", 76, smallFont)
    drawCentered(step.label, 102, systemFont)
    local progress = AssemblyWorkstation.getQteProgress()
    if #step.tokens > 1 then
        drawCentered(progress .. " / " .. #step.tokens, 140, smallFont)
        drawCentered("QUICK COMBO", 164, smallFont)
    else
        drawCentered("NOW!", 148, smallFont)
    end
end
...
function AssemblyWorkstation.draw()
    ...
    elseif completionPending then
        gfx.drawText("FINISHING...", 130, 211)
    else
        gfx.drawText("FOLLOW THE QTE", 130, 211)
    end
