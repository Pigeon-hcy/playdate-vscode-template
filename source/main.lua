...
-- The launcher shows this image before Lua runs. Flush it here as well for
-- simulator/direct launches, before importing modules that load game assets.
do
    local loadingImage = assert(playdate.graphics.image.new("SystemAssets/launchImage"))
    playdate.graphics.setDrawOffset(0, 0)
    playdate.graphics.clearClipRect()
    playdate.graphics.setImageDrawMode(playdate.graphics.kDrawModeCopy)
    loadingImage:draw(0, 0)
    playdate.display.flush()
end
import "gameVersions"
-- Imports run only at startup. Never load another version into live globals.
if GameVersions.getId() == "variantA" then
    import "variants/variantA/main"
    return
elseif GameVersions.getId() == "variantB" then
    import "variants/variantB/main"
    return
elseif GameVersions.getId() == "variantC" then
    import "variants/variantC/main"
    return
elseif GameVersions.getId() == "variantD" then
    import "variants/variantD/main"
    return
end
import "CoreLibs/ui"
